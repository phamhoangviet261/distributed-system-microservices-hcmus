                                    ...
                                    return (
                                        <tr style={{height: '60px', borderBottom: 'solid 1px #d7d7d7', cursor: 'pointer'}} key={index}>
                                            ...
                                            <td style={{textAlign: 'right'}}>
                                                <DefaultBtn onClick={() => handleSetDefault(item.maDC, userID)}>Đặt mặc định</DefaultBtn>
                                            </td>
                                        </tr>
                                    );
                                }
                    ...
                    >
                        {listDistrictCurrent.map((item, index) => {
                            if (item.maQuanHuyen === districtID)
                                return (
                                    <option value={item.maQuanHuyen} selected>
                                        {item.tenQuanHuyen}
                                    </option>
                                );
                            else return <option value={item.maQuanHuyen}>{item.tenQuanHuyen}</option>;
                        })}
                    </SelectTag>
                </FormGroup>
                <FormGroup>
                    ...
                    >
                        {listWardCurrent.map((item, index) => {
                            if (item.maPhuongXa === wardID)
                                return (
                                    <option value={item.maPhuongXa} selected>
                                        {item.tenPhuongXa}
                                    </option>
                                );
                            else return <option value={item.maPhuongXa}>{item.tenPhuongXa}</option>;
                        })}
                    </SelectTag>
                </FormGroup>
